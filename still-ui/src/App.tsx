...
<div className="window-controls" aria-label="Window controls">
 ...
 return <UICtx.Provider value={{state,setContext,open,arriving,setGhost}}><MotionConfig reducedMotion="user" transition={FLOW}><TooltipProvider delayDuration={650}>
  <div className={cn("browser-shell", !sidebar && "horizontal-layout", state.fullScreen && "is-fullscreen", entering && state.secondary && "window-enter", state.incognito && "is-incognito")} data-reduced-motion={!!reduced}>
   ...
   <header className="window-bar" onDoubleClick={e => { if (e.target === e.currentTarget) send("maximize") }} onPointerDown={e => { if (e.button === 0 && e.target === e.currentTarget) send("drag") }}>
    ...
    <div className="page-tools">
     ...
     {/^https:\/\/(chromewebstore\.google\.com|chrome\.google\.com\/webstore)\/detail\/(?:[^/?#]+\/)?[a-p]{32}(?:[/?#]|$)/.test(active?.url ?? "") && <Button size="sm" className="update-pill" onClick={() => send("storeExtension")} title="Download this extension from the Chrome Web Store and review its permissions before adding it."><Puzzle />Add to Still</Button>}
   ...
   <div className="workspace">
    ...
    <main className="page-column">
     ...
     {!state.focusMode && !state.fullScreen && state.bookmarks.length > 0 && <nav className={cn("bookmarks-bar", barOpen && state.bookmarks.length >= 5 && "is-expanded")} aria-label="Bookmarks bar">
      <div className="bookmarks-bar-list">{state.bookmarks.slice(0, barOpen ? 200 : 40).map(b => <BookmarkMenu key={b.url} b={b} onEdit={editBookmark}><Button variant="ghost" size="sm" className="bookmark-chip" title={b.title + " · " + b.url} onClick={() => send("navigate", { url: b.url })}><span className="bookmark-letter">{b.favicon ? <img src={b.favicon} alt="" /> : (host(b.url)[0] ?? "•").toUpperCase()}</span><span className="bookmark-name">{b.title || host(b.url)}</span></Button></BookmarkMenu>)}</div>
     ...
     <div className="page-slot" ref={pageRef}>
      {active?.url ? (snapshot && <img className="page-snapshot" src={snapshot} alt="" />) : <motion.div key={active?.id} className="new-tab-page" initial={{ opacity: 0, y: reduced ? 0 : 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: reduced ? .01 : .38, ease: [.22, 1, .36, 1] }}>
       ...
       {!active?.isPrivate && state.bookmarks.length > 0 && <div className="speed-dial">{state.bookmarks.slice(0, 8).map(b => <BookmarkMenu key={b.url} b={b} onEdit={editBookmark}><button className="speed-dial-tile" title={b.url} onClick={() => send("navigate", { url: b.url })}><span className="speed-dial-letter">{b.favicon ? <img src={b.favicon} alt="" /> : (host(b.url)[0] ?? "•").toUpperCase()}</span><span className="speed-dial-name">{b.title || host(b.url)}</span></button></BookmarkMenu>)}</div>}
  ...
  {state.welcome && <Welcome layout={state.preferences.layout} isDefault={state.preferences.isDefaultBrowser} send={send} onImport={() => open("import")} />}
  ...
  <Dialog open={dialogOpen} onOpenChange={o => { if (!o) close() }}>
   <DialogContent className={cn("app-dialog", displayPane === "settings" && "settings-dialog", ["history", "bookmarks", "cookies", "passwords", "extensions", "downloads", "security", "import", "profiles"].includes(displayPane) && "library-dialog")}>

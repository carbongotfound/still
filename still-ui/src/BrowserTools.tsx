...
export type ToolData={external?:{busy:boolean;error:string;done:string;profile:string;sources:{id:string;name:string}[];preview?:{id:string;name:string;bookmarks:number;history:number;passwords:number;cookies:number;cookiesLocked:boolean;skipped:number;warnings:string[]}};error?:string;vault?:{id:string;origin:string;username:string}[];cookies?:{id:string;name:string;domain:string;path:string;secure:boolean;httpOnly:boolean;session:boolean}[];extensions?:{id:string;name:string;enabled:boolean;hasPage:boolean;officialVersion?:string}[];candidate?:{name:string;permissions:string[]};permissions?:{origin:string;kind:string;state:string}[];engine?:string;isPrivate?:boolean;profiles?:{id:string;name:string;current:boolean;launch:boolean;permanent:boolean;running:boolean}[];preview?:{id:string;file:string;kind:string;logins:number;bookmarks:number;history:number;tabs:number;skipped:number};offer?:{id:string;origin:string;username:string;update:boolean};passwordOptions?:{save:boolean;fill:boolean;update:boolean};downloading?:boolean;official?:{version:string;sha256:string}}
export function BrowserTools({pane,data,send,url,tracking,memory,autofill}:{pane:string;data:ToolData;send:Send;url:string;tracking:string;memory:boolean;autofill:boolean}){
 ...
 return <div className="browser-tools">
  ...
  {pane==="extensions"&&<>
   <div className="tool-intro"><Puzzle/><p>Add unpacked Chromium extensions from a folder. Store installation and some browser-specific APIs are unavailable.</p></div>
   <div className="official-extension"><ShieldCheck/><div><strong>uBlock Origin Lite</strong><p>Official publisher release. Blocks ads and trackers using the browser’s filtering engine.</p><small>YouTube ad blocking and full compatibility in Still have not been verified.</small><div className="tool-toolbar"><Button variant="outline" disabled={data.downloading} onClick={()=>send("officialBlocker")}>{data.downloading?"Downloading…":data.extensions?.some(e=>e.officialVersion)?"Check for update":"Review install"}</Button><Button variant="ghost" onClick={()=>send("officialSource")}>Publisher<ExternalLink/></Button></div></div></div>
   <div className="tool-toolbar"><Button variant="outline" onClick={()=>send("extensionChoose")}><FolderOpen/>Load extension folder</Button><Button variant="ghost" size="icon-sm" aria-label="Refresh extensions" onClick={refresh}><RotateCw/></Button></div>
   {data.candidate&&<div className="extension-review"><strong>Add {data.candidate.name}?</strong>{data.official&&<p>Official release {data.official.version} · SHA-256 verified</p>}<p>Declared permissions and website access:</p><ScrollArea className="permission-list">{data.candidate.permissions.length?data.candidate.permissions.map(p=><code key={p}>{p}</code>):<span>No permissions requested.</span>}</ScrollArea><div className="tool-toolbar"><Button variant="ghost" onClick={()=>send("extensionCancel")}>Cancel</Button><Button onClick={()=>send("extensionInstall")}>Add extension</Button></div></div>}
  ...
  {pane==="cookies"&&<>
   ...
   <ScrollArea className="tools-list">{data.cookies?.filter(c=>(c.domain+c.name).toLowerCase().includes(filter.toLowerCase())).map(c=><div className="tool-row" key={c.id}><Cookie/><div className="tool-copy"><strong>{c.domain}</strong><small>{c.name} · {c.httpOnly?"HTTP only · ":""}{c.secure?"Secure":"HTTP"}{c.session?" · Session":""}</small></div><Button variant="ghost" size="icon-sm" aria-label={"Delete cookie "+c.name} onClick={()=>send("cookieDelete",{id:c.id})}><Trash2/></Button></div>)}{data.cookies?.length===0&&<p className="empty-state">No cookies in this profile.</p>}</ScrollArea>
  </>}
  {pane==="security"&&<>
